...
*/
	...
	class simpleMemcache { 
		...
		function cacheRead($key) {
			if($this->_memcache->get($key)) {
				$this->_lastValue = $this->_memcache->get($key);
		...
		function cacheWrite($t, $key, $v) {
			$key = $t . $key;
			$this->_memcache->set($key, $v, false, $this->_TTL);
		}
		
		function update() {
			$c = ob_get_contents();
			$key = $_SERVER['REQUEST_URI'];
			$this->cacheWrite('',$key, $c);
		}
	...
	// Cache full pages to the filesystem
	class simpleCache {
	...
	// cache data objects to the filesystem
	class objectCache {
		private $_cacheFileName;
		private $_updateTime;
		private $_objectName;
	...
	// cache mechanism for PORTIONS of pages to the filesystem
	class fragmentCache {
